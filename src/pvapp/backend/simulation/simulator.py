...
class Simulator:
    ...
    -------------
    >>> from pathlib import Path
    >>> sim = Simulator(subfolder=Path("./my_plant"))
...
#                         system=pvsydsystem.getplant(), site=self.site.location
#                     )
#                 except Exception as e:
#                     self.logger.warning(
...
#     def plant_name(self):
#         assert self.site is not None, "Site must be defined to get plant name."
#         assert self.pv_setup_data is not None, "Plant data must be defined to get"
#         return f"[{self.site.name} : {self.pv_setup_data["name"]}]"
