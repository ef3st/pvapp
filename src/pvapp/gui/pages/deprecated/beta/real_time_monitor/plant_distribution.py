...
def plant_distribution():
    ...
    with a.container(border=False):
        ...
        plant_report(current["messages"])
    with b:
        top_display(status)
        plant_map()
    buildnet, monitoring, manager = st.tabs(
        ["🏗️ Plant Network", "💻 Monitoring", "⚙️ Manager"],
    )
    with buildnet:
...
def plant_report(messages):
    with st.expander("📃 Plant Report", expanded=True):
