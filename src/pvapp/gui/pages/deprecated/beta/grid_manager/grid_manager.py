...
class GridManager(Page):
    ...
    def render(self):
        ...
        # Select Plant
        with ll:
            with st.expander(f" 🔎 {self.T("subtitle.search_plant")}", expanded=True):
    ...
    def select_plant(self):
        ...
        selected_site = col1.selectbox(
            f"🌍 {self.T("subtitle.site")}", sorted(plants_df["site_name"].unique())
        )
        filtered = plants_df[plants_df["site_name"] == selected_site]
        selected_plant = col2.selectbox(
            f"⚙️ {self.T("subtitle.plant")}", filtered["plant_name"]
        )
