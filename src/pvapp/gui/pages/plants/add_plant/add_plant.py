...
def get_coordinates(address: str):
    geolocator = Nominatim(user_agent="pvapp-app")
...
def init_session():
    if "plant_step" not in st.session_state:
        st.session_state.plant_step = 0
    if "new_plant" not in st.session_state:
        st.session_state.new_plant = {"site": {}, "plant": {}}
...
def step_site():
    ...
    with open("src/pvapp/gui/pages/plants/add_plant/districts.json") as f:
...
def navigation_buttons(step_back, next_key, target_dict, section_key, update_dict):
    ...
    with col2:
        if st.button("🔙 Back", key=f"{next_key}_back"):
            st.session_state.plant_step = step_back - 1
            st.rerun()
    with col3:
...
def render():
    ...
    if step == 0:
        ...
        st.markdown("🧰 **Plant**")
        name = st.text_input("Give a name to this plant", value="New Plant")
...
def steps():
    ...
    disableds[st.session_state.plant_step] = (
        False,
        icons[st.session_state.plant_step],
    )
    subtitles = [None for i in range(6)]
    new_plant = st.session_state.new_plant
    if "site" in new_plant:
        if "name" in new_plant["site"]:
            subtitles[0] = f"\n {new_plant["site"]["name"]}"
        if "coordinates" in new_plant["site"]:
            subtitles[1] = (
                f"\n({new_plant["site"]["coordinates"]["lat"]}, {new_plant["site"]["coordinates"]["lon"]})"
            )
    if "plant" in new_plant:
        if "module" in new_plant["plant"]:
            subtitles[2] = f"\n{new_plant["plant"]["module"]["name"]}".replace("_", " ")
        if "inverter" in new_plant["plant"]:
            subtitles[3] = f"\n{new_plant["plant"]["inverter"]["name"]}".replace(
                "_", " "
            )
        if "mount" in new_plant["plant"]:
            subtitles[4] = re.sub(
                r"(?<!^)(?=[A-Z])", " ", f"\n{new_plant["plant"]["mount"]["type"]}"
            )
