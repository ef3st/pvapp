    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                result = func(*args, **kwargs)
                execution_time = (datetime.now() - start_time).total_seconds()
                logger.info(f"Performance {operation_name}: {execution_time:.3f} seconds")
                return result
            except Exception as e:
                execution_time = (datetime.now() - start_time).total_seconds()
                logger.error(f"Performance {operation_name} failed after {execution_time:.3f} seconds: {str(e)}")
                raise
