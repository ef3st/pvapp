...
class GridManager(Page):
    ...
    def render(self):
        ...
        if not grid_file.exists():
            with rr:
                sac.result(label='NO GRID', description=self.T("messages.no_grid"), status='empty')
    ...
    def tabs(self):
        ...
        tab = sac.tabs([sac.TabsItem(label=titles[tab]["title"], tag=f"{tags[tab]}") for tab in titles
    ...
    def links_manager(self):
        ...
        with st.expander(self.T(f"{labels_root}.new_item"), icon="➕"):
            ...
            item = sac.chip(
                items = [
                    sac.ChipItem(items[i]["name"]) for i in items
                    ], label=self.T(f"{labels_root}.select_item"), index=[0, 2], radius='md', variant='light', return_index=True
            )
            if item == 0:
                self.add_bus()
            elif item == 1:
    ...
    def add_bus(self):
        ...
        n_new_bus = buttons_cols[0].number_input("n", label_visibility="collapsed",step=1,value=1,min_value=1)
    ...
    def bus_params(self,borders:bool = True, bus:Optional[BusParams] = None):
        labels_root = "tabs.links.item.bus"
        if not bus:
            bus:BusParams = BusParams(name = "New_Bus", vn_kv=0.230, type="b",in_service="True")   
        with st.container(border=borders):
            ...
            with sectors[0]:
                ...
                bus["name"] = st.text_input("Name", label_visibility="collapsed",value=bus["name"])
                type_idx = bidict({
                    "b": 0,
                    "n": 1,
                    "m": 2
                })                   
                bus["type"] = type_idx.inv[sac.segmented(items=[
                    sac.SegmentedItem(label=name) for name in self.T(f"{labels_root}.bus_level")
                ],direction="vertical", color="grey",index=type_idx[bus["type"]], return_index=True, align="center")]
                bus["in_service"] = sac.switch(self.T(f"{labels_root}.in_service"),value=bus["in_service"], position='left', align="center")
            
            with sectors[1]:
                sac.divider(label=titles[1], align="center")
                left, right = st.columns(2)
                with left:
                    voltage = sac.segmented(items=[
                        sac.SegmentedItem(label=name) for name in self.T(f"{labels_root}.voltage")
                    ],direction="vertical", color="grey", align="center", return_index=True)
                    voltage_type = bidict({
                        "LV": 0,
                        "MV": 1,
                        "HV": 2,
                    ...
                    bus["vn_kv"] = st.number_input("Test", label_visibility="collapsed", disabled=True, value=voltages[voltage_type.inv[voltage]])
                ...
                with right:
                    ...
                    min = st.number_input(names[1], value=contraints[0], disabled=disabled)
                    max = st.number_input(names[2], value=contraints[1], disabled=disabled)
                    if not disabled:
                        bus["min_vm_pu"] = min
                        bus["max_vm_pu"] = max
    ...
    def build_links(self,borders:bool = True, bus:Optional[BusParams] = None) -> Tuple[bool, LineParams]:
        labels_root = "tabs.links.item.link"
        with st.container(border=borders):
            ...
            def select_bus(align="start", name=None):
                ...
                with c:
                    ...
                    name = st.selectbox(label="Bus name",label_visibility="collapsed",options=list(self.grid.net.get("bus")["name"]), key=f"{align}_{name}Bus name")
                with d:
                    sac.divider(self.T(f"{labels_root}.bus_identity")[1], align=align)
                    index = st.number_input(label="Bus Index",label_visibility="collapsed",disabled=True, 
                                            value=self.grid.get_element("bus", name = name, column="index"), key=f"{align}_{name}Bus Index")
                with b:
                    map_level = {"b":0,"n":1,"m":2}
                    level_index = self.grid.get_element(element="bus", index=index,column="type")
                    if level_index:
                        level_index = map_level[level_index]
                    sac.segmented(
                        items=[sac.SegmentedItem(name) for name in self.T(f"{labels_root}.bus_level")], 
                        align="center", color='gren', size='sm',key=f"{align}_{name}_level_bus", disabled=True, index=level_index) 
                return index
                
            with first:
                sac.divider(self.T(f"{labels_root}.buses")[0], align="center", variant="dashed")
                start_bus = select_bus(name="1a")
            with second:
                sac.divider(self.T(f"{labels_root}.buses")[1], align="center", variant="dashed")
                end_bus = select_bus("end", "1b")
            with link:
                label_line_params = self.T(f"{labels_root}.line_params")
                a,b,c = st.columns([2,1,2])
                type = a.selectbox(label_line_params[0],options=self.grid.get_aviable_lines())
                length = b.number_input(label=f"{label_line_params[1]} (km)" ,value=0.1)
                name = c.text_input(label_line_params[2],value=f"New_({type})_{length}km")
                error_map = self.T(f"{labels_root}.errors")
                color = "green"
                buses = self.grid.net.bus
                link_aviable = True
                error = self.grid.aviable_link(buses.iloc[start_bus],buses.iloc[end_bus]) 
                if error:
                    color = "red"
                    link_aviable = False
                sac.divider(error_map[error], align="center",size=5,color=color,variant="dotted")
            ...
            new_link = LineParams(from_bus=start_bus,to_bus=end_bus,length_km=length,name=name,std_type=type)
