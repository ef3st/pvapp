...
def status_panels():
    ...
    with st.expander("Inverter Info: 🟢 Online"):
        ...
        """.format(df["Power"].sum(), pd.Timestamp.now().strftime("%H:%M:%S")))
    ...
    cols = st.columns(int(N_STRINGS / 2))  # 3 colonne per ogni stringa (sinistra | modulo | destra)
            ...
            for m in range(MODULES_PER_STRING):
                ...
                with left:
                    st.markdown(
                        ...
                        f"T:{left_mod['Temperature']:.0f}</div>", unsafe_allow_html=True)
                ...
                with right:
                    st.markdown(
                        ...
                        f"T:{right_mod['Temperature']:.0f}</div>", unsafe_allow_html=True)
...
def visulization_implant():
    ...
    labels = go.Scatter3d(
        x=[floor_x[2], floor_x[2], floor_x[2]+0.5, floor_x[2]-0.5],  # Est-Ovest sui +X/-X
        y=[floor_y[2]+0.8, floor_y[2]-0.8, floor_y[2], floor_y[2]],  # Nord-Sud sui +Y/-Y
        z=[.03, 0.03, 0.03, 0.03],  # Pavimento (z=0)
        mode="text",
        text=["S", "N", "E", "O"],
...
def three_point_angle(A, B, C, geographic = True):
    ...
    angolo_rad = np.arccos(np.clip(cos_angolo, -1.0, 1.0))  # clip per stabilità numerica
